...
//因为第一位为符号位，正数为0，值即绝对值，负数首位为1，其值为取反加一。
        ...
        for(double x=0;x!=10;x+=0.1){
            //在判断浮点数是否相等时要格外小心，由于舍入的误差，可能永远无法变成10，因为0.1无法精确地用二进制表示
        ...
        char x = 'A';
        /**
         //char类型的值可以表示为十六进制值,使用单斜杠u可以用十六进制表示char,\u03C0表示希腊字母π
         //public static void main(String\u005B\u0050 args)
         //  单斜杠u 00A0 is a newline。这时候会替换成一个换行符
         //在注释中出现单斜杠u的时候要注意 比如 C:单斜杠user\这时候可能会报错。
...
//boolean类型
        ...
        //类型转换
        //当两个不同类型的数值进行二元操作时，先要将操作数转换成一种类型
        //double > float >long >int
        char ch='A';
        int in=1;
        //int转换成float,long转换成float和double时，可能会损失精度

        //强制类型转换
        double xx=9.97;
        int yy=(int)xx;
        //如果要使用舍入运算，更好地是使用Math.round()

        //如果讲一个数值强制转换成另一种类型，并且超出了目标类型的表示范围，结果会变成一个完全不同的值，
        byte zz=(byte)300;
        //实际值为44
